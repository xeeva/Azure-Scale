...
class GraphHtmlGenerator:
    ...
    def _build_data_js(self) -> str:
        ...
        # ---- AUTO-CONNECT ORPHANS TO CATEGORY HUBS ----
        connected_ns = set()
        for src_ns, tgt_ns, _ in self._PARENT_RELATIONSHIPS:
            if src_ns in self._ns_set:
                connected_ns.add(src_ns)
            if tgt_ns in self._ns_set:
                connected_ns.add(tgt_ns)
        for p in self.providers:
            if p.sub_services:
                connected_ns.add(p.namespace)

        # Find highest-score connected provider per category as hub
        cat_hubs: dict[str, str] = {}
        for p in sorted(
            self.providers, key=lambda x: x.complexity_score, reverse=True
        ):
            cat = self._categorize(p.namespace)
            if cat not in cat_hubs and p.namespace in connected_ns:
                cat_hubs[cat] = p.namespace

        # Connect each orphan to its category hub
        for p in self.providers:
            if p.namespace in connected_ns:
                continue
            cat = self._categorize(p.namespace)
            hub_ns = cat_hubs.get(cat)
            if hub_ns and hub_ns in self._ns_set:
                src_id = self._ns_to_id[p.namespace]
                tgt_id = self._ns_to_id[hub_ns]
                parent_edges.append(
                    f"    ['{self._js_str(src_id)}',"
                    f"'{self._js_str(tgt_id)}',"
                    f"0.2]"
    ...
    def _inject_features(self, html: str) -> str:
        ...
        # Inject extra JS before the final closing </script> (not JSON-LD one)
        if extra_js:
